...
import videosMascot from "@/assets/characters/videos.png";
...
export const Route = createFileRoute("/resources/videos")({
  head: () => ({
    meta: [
      { title: "Videos — Girls Leading Tech" },
      { name: "description", content: "YouTube channels, lectures and playlists worth bingeing." },
    ],
  }),
...
function VideosPage() {
  ...
  const _filtered = filterBySearch(videos, _q, ["title","name","description","summary","author","provider","organisedBy","company","role","domain","category","categories","benefit","eligibility","keywords"]);
  return (
    <div className="relative w-full min-h-screen bg-[#fef9f4] overflow-hidden">
            ...
            <div className="flex flex-col-reverse md:flex-row items-center md:items-stretch justify-between gap-8 md:gap-12 relative z-10">
              ...
              <div className="flex-1 text-center md:text-left flex flex-col justify-center animate-fade-up">
                ...
                <h1 className="font-sans text-4xl sm:text-5xl md:text-6xl font-bold text-gray-900 leading-tight tracking-tight">
                  Watch, learn, <span className="italic font-medium text-[#5b2b4a] font-serif">repeat.</span>
                </h1>
                <p className="mt-4 max-w-xl text-sm md:text-base leading-relaxed text-muted-foreground">
                  The YouTube channels, lectures and playlists every woman in tech should bookmark.
                </p>
              </div>
      ...
      <section className="relative z-10 container mx-auto max-w-6xl px-6 pb-24 pt-4">
        <ResourceSearchBar value={_q} onChange={_setQ} placeholder="Search videos..." />
