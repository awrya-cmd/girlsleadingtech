...
import roadmapMascot from "@/assets/characters/roadmap.png";
...
export const Route = createFileRoute("/resources/roadmaps")({
  head: () => ({
    meta: [
      { title: "Roadmaps — Girls Leading Tech" },
      { name: "description", content: "Visual roadmaps for every tech career path." },
    ],
  }),
...
function RoadmapsPage() {
  ...
  const _filtered = filterBySearch(roadmaps, _q, ["title","name","description","summary","author","provider","organisedBy","company","role","domain","category","categories","benefit","eligibility","keywords"]);
  return (
    <div className="relative w-full min-h-screen bg-[#fef9f4] overflow-hidden">
        ...
        <div className="container mx-auto max-w-6xl relative">
          ...
          <div className="relative bg-[#FFF8EF] border-2 border-black rounded-[24px] pt-16 pb-8 px-6 md:pt-20 md:pb-12 md:px-12 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-visible">
            ...
            <img
              ...
              className="absolute -top-7 left-[25%] w-28 rotate-[4deg] pointer-events-none z-20 select-none transition-transform hover:scale-105"
            />
            <img
              src={star}
              alt="Star Sticker"
              className="absolute -top-7 -right-4 w-12 rotate-[-10deg] pointer-events-none z-20 select-none transition-transform hover:scale-110"
            />
            ...
            <div className="flex flex-col-reverse md:flex-row items-center md:items-stretch justify-between gap-8 md:gap-12 relative z-10">
              ...
              <div className="flex-1 text-center md:text-left flex flex-col justify-center animate-fade-up">
                ...
                <h1 className="font-sans text-4xl sm:text-5xl md:text-6xl font-bold text-gray-900 leading-tight tracking-tight">
                  Career <span className="italic font-medium text-[#5b2b4a] font-serif">playbooks.</span>
                </h1>
                <p className="mt-4 max-w-xl text-sm md:text-base leading-relaxed text-muted-foreground">
                  Step-by-step visual paths for every role in tech — frontend, AI, product, design and more.
                </p>
              </div>
      ...
      <section className="relative z-10 container mx-auto max-w-6xl px-6 pb-24 pt-4">
        <ResourceSearchBar value={_q} onChange={_setQ} placeholder="Search roadmaps..." />
